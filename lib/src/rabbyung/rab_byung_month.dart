...
class RabByungMonth extends AbstractTyme {
  ...
  static const List<String> alias = [
    '神变月', '苦行月', '具香月', '萨嘎月', '作净月', '明净月',
    '具醉月', '具贤月', '天降月', '持众月', '庄严月', '满意月'
  ];

  /// 藏历年
  final RabByungYear year;
  ...
  RabByungMonth(this.year, int month)
      : month = month.abs(),
        leap = month < 0 {
  ...
  static RabByungMonth fromYm(int year, int month) {
    return RabByungMonth(RabByungYear.fromYear(year), month);
  }

  static RabByungMonth fromElementZodiac(
      int rabByungIndex, RabByungElement element, Zodiac zodiac, int month) {
    return RabByungMonth(
        RabByungYear.fromElementZodiac(rabByungIndex, element, zodiac), month);
  }

  /// 藏历年
  RabByungYear getRabByungYear() => year;
  ...
  @override
  String toString() => year.toString() + getName();
  ...
  /// 首日
  RabByungDay getFirstDay() => RabByungDay(this, 1);
